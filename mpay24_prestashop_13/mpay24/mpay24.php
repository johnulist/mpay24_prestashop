		...
		if (Tools::isSubmit('submitMpay24Checkout'))
		{
			...
			if (($merchant_id = Tools::getValue('mpay24_merchant_id')) AND (preg_match('/7[0-9]{4}/', $merchant_id)
				...
				for($i=0; $i<$result->getAll(); $i++){
					$mpay24PaymentSystems.= $result->getBrand($i) . "=>" . $result->getPType($i) . ",";
				}
				if($result->getGeneralResponse()->getStatus() == 'OK'){
					Configuration::updateValue("MPAY24_ORDER_S", "margin-left: auto; margin-right: auto; width: 600px;");
...
// 					}
				...
				$errors[] = Tools::displayError("<div class='warn error'><h1 style='color: red;'>".$this->l('The merchant ID seems to be wrong! It should be a 5 digit number,
						starting with 7 or 9.').'</h1></div>');
		...
		if (Tools::isSubmit('submitPaymentSystems')){
			...
			foreach ($_POST as $key => $value){
				if(substr($key, 0, 3) == "ps_")
					$paymentSystems.= substr($key, 3)."=>$value,";
			}
			...
			if(Tools::getValue('MPAY24_PAYMENT_SYSTEMS_ENABLED') == 'true')
				...
				$activePaymentSystems = explode(",", Configuration::get('MPAY24_ACTIVE_PAYMENT_SYSTEMS'));

				$checkedPaymentSystems = explode(",", Configuration::get('MPAY24_PAYMENT_SYSTEMS_CHECKED'));
				foreach($checkedPaymentSystems as $cps){
					if(in_array($cps, $activePaymentSystems)){
						if(($key = array_search($cps, $activePaymentSystems)) !== false) {
							unset($activePaymentSystems[$key]);
						}
					}
				}
				...
				foreach($activePaymentSystems as $aps){
					$a = explode("=>", $aps);
					$shopActivePaymentSystems.= $a[0]."=>".$a[1].",";
				}
...
// 		}
		...
		<br /><br />';
		$brandsAndPtypes = explode(",", Configuration::get('MPAY24_ACTIVE_PAYMENT_SYSTEMS'));
		$brands = array();
		$ptypes = array();

		foreach($brandsAndPtypes as $brandAndPtype) {
			$x = explode("=>", $brandAndPtype);
			array_push($brands, $x[0]);
			array_push($ptypes, $x[1]);
		}
...
// 			</div>
	...
	function hookPayment($params) {
		...
		foreach ($ps as $p)
			if($p != '') {
			$x = explode("=>", $p);
				$FEpaymentSystems.= '<img src="https://www.mpay24.com/web/img/logos/brands/small/'.$x[0].'.png" hspace="2" vspace="2" alt="'.$x[0].'" title="'.$x[0].'">';
				}
	...
	return bool;
		...
		if ($module['module'] == 'mpay24') {
			$url = $_SERVER['PHP_SELF'] . "?controller=".Tools::getValue('controller')."&token=" . Tools::getValue('token') . "&vieworder&id_order=" . $result['id_order'];
			...
			if (isset($_POST['updateStatus'])) {
				...
				if($r->getGeneralResponse()->getStatus() != "OK")
					$url .= "&error=" . urlencode($r->getGeneralResponse()->getReturnCode());
				if($r->getParam('USER_FIELD') == PRESTASHOP_MODULE_VERSION . ' ' . $secure_key)
			...
			switch($trans['STATUS']){
				case 'RESERVED':
					...
					$msg .='<br><br><form action="index.php?controller='.Tools::getValue('controller').'&id_order='.Tools::getValue('id_order').'&vieworder&token='.Tools::getValue('token').'" method="post" name="clear_credit" style="text-align:center;">
					...
					$msg .='</form><form action="index.php?controller='.Tools::getValue('controller').'&id_order='.Tools::getValue('id_order').'&vieworder&token='.Tools::getValue('token').'" method="post" name="cancel" style="text-align:center;">
					...
					$msg .='</form><form action="index.php?controller='.Tools::getValue('controller').'&id_order='.Tools::getValue('id_order').'&vieworder&token='.Tools::getValue('token').'" method="post" name="updateStatus" style="text-align:center;">
				...
				case 'BILLED':
					...
					<br /><br /><center></form><form action="index.php?controller='.Tools::getValue('controller').'&id_order='.Tools::getValue('id_order').'&vieworder&token='.Tools::getValue('token').'" method="post" name="cancel" style="text-align:center;">
				...
				case 'CREDITED':
					...
					<br /><br /><form action="index.php?controller='.Tools::getValue('controller').'&id_order='.Tools::getValue('id_order').'&vieworder&token='.Tools::getValue('token').'" method="post" name="updateStatus" style="text-align:center;">
		...
		<tr>
		<td>MPAYTID:</td>
		<td>".$mpay24OrderDetails['MPAYTID']."</td>
		</tr>
		<tr>
		<td>".$this->l('Payment system').":</td>
		<td>".$mpay24OrderDetails['P_TYPE']."</td>
		</tr>
		<tr>
		<td>Brand:</td>
		<td>
		<img src='https://www.mpay24.com/web/img/logos/brands/small/".$mpay24OrderDetails['BRAND'].".png' title='".$mpay24OrderDetails['BRAND']."'/>
		</td>
		</tr>
		<tr>
		<td>".$this->l('Approval code').":</td>
		<td>".$mpay24OrderDetails['APPR_CODE']."</td>
		</tr>
		</tbody>
		</table>
		</div>
