...
class mpay24 extends PaymentModule {
	...
	public function getContent() {
		...
		if (Tools::isSubmit('submitPaymentSystems')){
			...
			if(Tools::getValue('MPAY24_PAYMENT_SYSTEMS_ENABLED') == 'true')
				...
				foreach($activePaymentSystems as $aps){
					$shopActivePaymentSystems.= explode("=>", $aps)[0]."=>".explode("=>", $aps)[1].",";
				}
		...
		foreach($brandsAndPtypes as $brandAndPtype) {
			array_push($brands, explode("=>", $brandAndPtype)[0]);
			array_push($ptypes, explode("=>", $brandAndPtype)[1]);
		}
		...
		if((Tools::isSubmit('submitMpay24Checkout') && $_REQUEST["resultStatus"] == 'OK') || Configuration::get("MPAY24_ACTIVE_PAYMENT_SYSTEMS") != ''){
			...
			for($i=0; $i<Configuration::get("MPAY24_ALL_ACTIVE_PS"); $i++){
				...
				foreach(explode(",", Configuration::get("MPAY24_PAYMENT_SYSTEMS_CHECKED")) as $checkedPS)
					array_push($brandsArray, explode("=>", $checkedPS)[0]);
...
// 			</div>
	...
	function hookPayment($params) {
		...
		foreach ($ps as $p)
			if($p != '')
				$FEpaymentSystems.= '<img src="https://www.mpay24.com/web/img/logos/brands/small/'.explode("=>", $p)[0].'.png" hspace="2" vspace="2" alt="'.explode("=>", $p)[0].'" title="'.explode("=>", $p)[0].'">';
